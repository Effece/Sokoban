...
def get_score():
    ...
    if clock.activated:
        s += 50
        s += r.max_t - int((time.time() - timer.t))
...
def show_stars(nb):
    ...
    """
    In :
      nb : le nombre d'étoiles à afficher
    """

    global stars

    for k in range(-1,nb-1):
        stars.append(g.obj_design(can, 76, can.width/2+k*(r.rap*0.75+r.des)-r.rap*0.75/2, can.height-r.des/3, can.width/2+k*(r.rap*0.75+r.des)+r.rap*0.75/2, can.height-r.des/3-r.rap*0.75))
